...
class WilcoInvoiceSummaryWizard(models.TransientModel):
    ...
    def _generate_by_sales_account(self, invoices, opening_month_int):
        ...
        for account_id, opening_before in account_opening_before.items():
            ...
            historical_period = self.env['wilco.customer.invoice.summary'].create({
                ...
                'balance': opening_before['sales_amount'] - opening_before['settled_amount'],
        ...
        for key in sorted_keys:
            ...
            period_summary = self.env['wilco.customer.invoice.summary'].create({
                ...
                'balance': current_data['sales_amount'] - current_data['settled_amount'],
    ...
    def _generate_generic_report_data(self, invoices, opening_month_int, group_by='period'):
        ...
        :param group_by: String indicating how to group data ('period', 'customer')
        """
        self.ensure_one()
        ...
        for invoice in invoices:
            ...
            if is_opening_period:
                ...
                if opening_key not in opening_data_before:
                    partner_id = invoice_customer_id if group_by == 'customer' else self.partner_id.id if self.partner_id else False
                ...
                opening_data_before[opening_key]['sales_amount'] += invoice.amount_total
                opening_data_during[opening_key]['sales_amount'] += invoice.amount_total
        ...
        for opening_key, opening_before in opening_data_before.items():
            ...
            # Second row: Opening period (settlement during opening period)
            opening_period_balance = period_balances[opening_key] - opening_during['settled_amount']
        ...
        for key in sorted_keys:
            ...
            # Calculate period balance: previous period balance + current sales - current settled
            new_period_balance = period_balances[group_key] + current_sales - current_settled
            ...
            period_summary = self.env['wilco.customer.invoice.summary'].create({
                ...
                'settled_amount': current_data['settled_amount'],
                'balance': current_data['sales_amount'] - current_data['settled_amount'],
                'period_balance': new_period_balance,  # Running period balance for this group
                'as_of_date': self.as_of_date,
                'partner_id': current_data['partner_id'],
                'is_opening': False,
                'description': False,
